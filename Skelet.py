...
class AlphaBeta:
    ...
    def optimalna_poteza(self, pozicija, igralec):
        ...
        self.poteza = None # Sem napišemo potezo, ko jo najdemo
        # Poženemo minimax
        (poteza, vrednost) = self.alphabeta(self.globina, -AlphaBeta.INFI, AlphaBeta.INFI, True)
        self.jaz = None
        self.pozicija = None
        if not self.prekinitev:
            # Potezo izvedemo v primeru, da nismo bili prekinjeni
            ...
            self.poteza = poteza
    #
    def alphabeta(self, globina, A, B, maximiziramo):
        if self.prekinitev:
            # Sporočili so nam, da moramo prekiniti
            logging.debug ("Minimax prekinja, globina = {0}".format(globina))
            return (None, 0)
        ...
        if konec:
            ...
            if globina == 0:
                ...
                if maximiziramo:
                    ...
                    #preverimo če imamo sploh potezo
                    if moznosti == None:
                        return self.alphabeta(globina,A,B, not maximiziramo)
                    else:
                        najboljsa_poteza = None
                        vrednost_najboljse = -MinMax.INFI
                        for p in moznosti:
                            self.pozicija.postavi(self.jaz, p)
                            vrednost = self.alphabeta(globina-1,A,B, not maximiziramo)[1]
                            self.pozicija = kopija
